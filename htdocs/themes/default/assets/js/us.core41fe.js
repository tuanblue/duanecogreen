		...
		if (!$us.$html.hasClass('no-touch')) {
			this.$list.find('.w-nav-item.menu-item-has-children.togglable > .w-nav-anchor').on('click', function(e){
				...
				var outsideClickEvent = function(e){
					if ($.contains($item[0], e.currentTarget)) return;
					$item.removeClass('opened');
					$list[hideFn]();
...
!function($){
	...
	USScroll.prototype = {
		...
		_countPosition: function(hash){
			this.blocks[hash].top = Math.ceil(this.blocks[hash].target.offset().top - $us.canvas.scrolledOccupiedHeight - this._canvasTopOffset);
		...
		scrollTo: function(place, animate){
			...
			// Scroll by hash
				...
				newY = Math.floor(place.offset().top - $us.canvas.scrolledOccupiedHeight - this._canvasTopOffset);
				placeType = 'element';
			}
			else {
				newY = Math.floor(place - $us.canvas.scrolledOccupiedHeight - this._canvasTopOffset);
			}
